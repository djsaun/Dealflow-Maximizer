...
add_action( 'add_meta_boxes', 'add_widget_box' );

function add_widget_box() {
    add_meta_box(
        'portfolio_widget', // ID, should be a string.
        'Featured Widget', // Meta Box Title.
        'portfolio_widget_meta_box', // Your call back function, this is where your form field will go.
        'x-portfolio', // The post type you want this to show up on, can be post, page, or custom post type.
...
function portfolio_widget_meta_box( $post ) {

	// Add a nonce field so we can check for it later.
	wp_nonce_field( 'portfolio_widget_save_meta_box_data', 'portfolio_widget_meta_box_nonce' );
	$widgetValue = get_post_meta( $post->ID, 'portfolio_widget', true );

	echo '<input type="text" id="portfolio_widget" name="portfolio_widget" style="width:100%;" value="' . esc_attr( $widgetValue ) . '" />';
}

function portfolio_widget_save_meta_box_data( $post_id ) {

	// Check if our nonce is set.
	if ( ! isset( $_POST['portfolio_widget_meta_box_nonce'] ) ) {
		return;
	}

	// Verify that the nonce is valid.
	if ( ! wp_verify_nonce( $_POST['portfolio_widget_meta_box_nonce'], 'portfolio_widget_save_meta_box_data' ) ) {
		return;
	}
	...
	// Make sure that it is set.
	if ( ! isset( $_POST['portfolio_widget'] ) ) {
		return;
	}

	// Sanitize user input.
	$my_data = sanitize_text_field( $_POST['portfolio_widget'] );

	// Update the meta field in the database.
	update_post_meta( $post_id, 'portfolio_widget', $my_data );
}
add_action( 'save_post', 'portfolio_widget_save_meta_box_data' );
